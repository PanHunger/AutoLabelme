...
def main():
    ...
    for filename in sorted(glob.glob(osp.join(args.input_dir, "*.json"))):
        ...
        label_file = libs.LabelFile(filename=filename)
        ...
        img = libs.utils.img_data_to_arr(label_file.imageData)
        imgviz.io.imsave(out_img_file, img)

        cls, ins = libs.utils.shapes_to_label(
            img_shape=img.shape,
        ...
        # class label
        libs.utils.lblsave(out_clsp_file, cls)
        if not args.nonpy:
        ...
        if not args.noobject:
            # instance label
            libs.utils.lblsave(out_insp_file, ins)
            if not args.nonpy:
