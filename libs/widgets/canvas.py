...
class Canvas(QtWidgets.QWidget):
    ...
    def initializeAiModel(self, name):
        if name not in [model.name for model in libs.ai.MODELS]:
            raise ValueError("Unsupported ai model: %s" % name)
        model = [model for model in libs.ai.MODELS if model.name == name][0]
        ...
        self._ai_model.set_image(
            image=libs.utils.img_qt_to_arr(self.pixmap.toImage())
        )
    ...
    def closeEnough(self, p1, p2):
        ...
        return libs.utils.distance(p1 - p2) < (self.epsilon / self.scale)
    ...
    def intersectingEdges(self, point1, point2, points):
        ...
        for i in range(4):
            ...
            if 0 <= ua <= 1 and 0 <= ub <= 1:
                ...
                d = libs.utils.distance(m - QtCore.QPointF(x2, y2))
                yield d, i, (x, y)
